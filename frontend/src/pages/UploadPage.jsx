        ...
        <div style={styles.results}>
          ...
          {result.analysis?.overall_summary && (
            <div style={styles.section}>
              ...
              <p><strong>Verdict:</strong> {result.analysis.overall_summary.verdict}</p>
              <p><strong>Confidence:</strong> {result.analysis.overall_summary.confidence_level}</p>
            ...
            <div style={styles.section}>
              ...
              {result.analysis.potential_issues.map((issue, index) => (
                <div key={index} style={styles.issue}>
                  ...
                  <p><span style={styles.badge}>{issue.severity}</span> {issue.type}</p>
                  <p>{issue.why_flagged}</p>
                  <p style={styles.action}><strong>Action:</strong> {issue.suggested_action}</p>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={styles.section}>
              ...
              {result.analysis.insurance_attention_items.map((item, index) => (
                <div key={index} style={styles.point}>
                  ...
                  <p>{item.reason}</p>
                  <p><strong>Coverage:</strong> {item.coverage_likelihood}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {result.analysis?.final_advice_for_patient?.length > 0 && (
            <div style={styles.section}>
              ...
              <ul>
                {result.analysis.final_advice_for_patient.map((advice, index) => (
